...
def MPS(bonds, input_size, legs):
    cells = []
    # first cell
    x = np.random.uniform(0, 1, size=(legs,bonds[0]) )
    first_cell = torch.from_numpy( x )
    cells.append(first_cell)
    for i in range(1, input_size - 1):
        y = np.random.uniform(0, 1,size=( legs,bonds[i - 1], bonds[i]) )
        cell = torch.from_numpy(y)
        cells.append(cell)
    # last cell
    i = input_size - 2
    z =  np.random.uniform(0, 1, size=(bonds[i],legs))
    last_cell = torch.from_numpy(z)
...
def get_identity_operator(input_size, legs):
    ...
    for i in range(input_size):
        temp = torch.eye(legs, dtype=torch.float64)
...
def operator_contraction_efficient(operator, bonds, cells, legs):
    ...
    for i in range(len(cells)):
        if i==len(cells)-1:
            temp = torch.tensordot(operator[i], cells[i], [0, 1])
        else:
            temp = torch.tensordot(operator[i], cells[i], [0, 0])
        bot_cells.append(temp)

    for i in range(len(cells)):
        try:
            if i ==len(cells)-1:
                contractions = torch.tensordot(contractions, bot_cells[i], [0, -1])
                contractions = torch.tensordot(contractions, top_cells[i], [[0, 1], [0, 1]])
            else:
                contractions = torch.tensordot(contractions, bot_cells[i], [0, -2])
                contractions = torch.tensordot(contractions, top_cells[i], [[0, 1], [-2, 0]])
        except:
            assert (i == 0)
            contractions = bot_cells[0]
            contractions = torch.tensordot(contractions, top_cells[i], [0, 0])
    return contractions
...
def layers_contraction(input_data, cells,measured_qubits):
    ...
    for image in input_data:
        ...
        for i,q in enumerate(measured_qubits):
            ...
            cell = cells[q]
            if q==max_qubits-1:
                temp = torch.tensordot(pixel, cell, [0, -1])
            else:
                temp = torch.tensordot(pixel, cell, [0, 0])
        ...
        for i in range(len(contracted_cells) - 1):
            try:
                if len(contracted_cells[i + 1].shape)==1:             # ---> it is just for when last qubit is measured. can be written nicer if the for loop is incremented reverse.
                    psi = torch.tensordot(psi, contracted_cells[i + 1], [-1, -1])
                else:
                    psi = torch.tensordot(psi, contracted_cells[i + 1], [-1, -2])
            except:
                assert (i == 0)
                psi = torch.tensordot(contracted_cells[i], contracted_cells[i + 1], [-1, -2])
...
def get_psi_squared(psi):
    ...
    psi_sqrt = torch.tensordot(psi, psi, [axes, axes])
    return psi_sqrt
...
def loss_function(bonds, input_data, legs, cells,measured_qubits,epsilon):
    ...
    loss = -(torch.sum(input_tensor=torch.log(psi_2) + epsilon) / len(psi_2)) + torch.log(z_tensor + epsilon)
    grads = loss.backward()
    return loss, grads
...
#     optimizer = torch.optim.RMSprop(learning_rate=5e-2,decay=0.5,momentum=0.5)
#     optimizer = torch.optim.SGD(learning_rate=5e-2)
